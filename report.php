...
class quiz_mcq_report extends quiz_default_report {
    ...
    protected function output_screen ($warnings, $nodata, $quiz, $reporturl, $groupnames, $filternames, $noofuserattempts,
        ...
        echo '<th class="header"><a href="' . qualified_me() . '&sort=' . $sortarg1 . '">'
            . get_string('firstname') . '</a></th>'
            . '<th class="header"><a href="' . qualified_me() . '&sort=' . $sortarg2 . '">'
            . get_string('lastname') . '</a></th>'
            . '<th class="header"><a href="' . qualified_me() . '&sort=' . $sortarg3 . '">'
            . get_string('grade', 'quiz') . '</a></th>';
            /*. '<th class="header"><a href="' . qualified_me() . '&sort=' . $sortarg4 . '">'
            . get_string('qattempts', 'quiz_mcq') . '</a> (%)</th>';*/

        foreach ($mcqkeys as $q) {
            echo '<th class="header"><a href="'.new moodle_url('/question/preview.php', array('id' => $mcqids[$q][0])).'" target="_blank">' . get_string('questionabbr', 'quiz_mcq') . ($q + 1) . '</a></th>';
        }
        echo '</tr>';
        // Summary rows
        echo '<tr>';
        echo '<td colspan="3" class="subsubheading">' . get_string('correctoptions', 'quiz_mcq') . '</th>';
        foreach ($mcqkeys as $q) {
        ...
        echo '</tr>';
        echo '<tr>';
        echo '<td colspan="3" class="subsubheading">' . get_string('totalcorrect', 'quiz_mcq') . '</th>';
        foreach ($mcqkeys as $q) {
        ...
        echo '</tr>';
        echo '<tr>';
        echo '<td colspan="3" class="subsubheading">' . get_string('totalchoosing', 'quiz_mcq') . '</th>';
        foreach ($mcqkeys as $q) {
        ...
        // Label
        echo '<tr class="optioncounts"><th colspan="' . ($noofmcqs + 3) . '" class="subheading">&nbsp;</th></tr><tr>';
        echo '<th colspan="' . ($noofmcqs + 3) . '" class="subheading">'
           . get_string('optioncounts', 'quiz_mcq') . ':<br /></th>';
        echo '</tr>';
        // Headers
        echo '<tr>';
        echo '<th colspan="3" class="header">' . get_string('option', 'quiz_mcq') . '</th>';
        foreach ($mcqkeys as $q) {
            echo '<th class="header qnum"><a href="'.new  moodle_url('/question/preview.php', array('id' => $mcqids[$q][0])).'" target="_blank">' . get_string('questionabbr', 'quiz_mcq') . ($q + 1) . '</a></th>';
        }
        echo '</tr>';
        // Data
        for ($opt = 0; $opt <= $maxoptions; $opt++) {
            echo '<tr>';
            if ($opt == 0) {
                echo '<td colspan="3">' . get_string('nochoice', 'quiz_mcq') . '</td>';
            } else {
                echo '<td colspan="3">' . $opt . '</td>';
            }
            foreach ($mcqkeys as $q) {
        ...
        if (count($uncompletedusers) > 0) {
            ...
            echo '<tbody>';
            echo '<tr>';
            echo '<th class="header">' . get_string('firstname', 'quiz_mcq') . '</th>'
                . '<th class="header">' . get_string('surname', 'quiz_mcq') . '</th>';
            echo '</tr>';
    ...
    protected function output_print ($warnings, $nodata, $quiz, $reporturl, $groupnames, $filternames, $noofuserattempts,
        ...
        for ($blk = 0; $blk < $noofblocks; $blk++) {
            ...
            foreach ($optiondata as $optiondatarow) {
                ...
                echo '<td>' . $optiondatarow[2] . '</td>';
                echo '<td>' . $optiondatarow[1] . '</td>';
                echo '<td class="numeric">' . $optiondatarow[3].'</td>';
               /* echo '<td class="numeric">' . $optiondatarow[4]
                    . ' (' . round(100 * $optiondatarow[4] / $noofmcqs) . '%)</td>';*/
